...
if(idade < 21 || idade > 55 || renda < 1000 || emprestimo < 500 || emprestimo >= renda * 15){
    ...
    let juros = 8.5 / 100;
    let total = emprestimo * (1 + juros); // M = C x (1+ i)
    if(parcelas <= 3){
        ...
        console.log(`O emprestimo no valor de R$${emprestimo}, dividido em ${parcelas} vezes com juros, terá um total de R$${total/parcelas} por mês!`);
    }
}